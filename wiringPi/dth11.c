...
int dht11_read_val(){
	...
	delayMicroseconds(40);							//start recieve dht response
	pinMode(DHT11PIN,INPUT);						//set pin to input
	for(i=0;i<MAX_TIME;i++){
		counter=0;
		while(digitalRead(DHT11PIN)==lststate){
			//read pin state to see if dht responsed. 
			//if dht always high for 255 + 1 times, break this while circle
			counter++;
			delayMicroseconds(1);
...
int main(void){
	...
	while(attempts){                        
		...
		//get result including printing out
		if(success){
